...
class Slider extends Component {
	...
	calculateSegments() {
		...
		if (this.props.range && this.props.range < 2) console.warn('Error: `range` cannot be less than 2, falling back to default 2.');
		else if (this.props.range)
			// eslint-disable-line
			safeRange = this.props.range;
		else if (this.props.data && this.props.data.length > 2) safeRange = this.props.data.length;
		...
		if (!this.state.isUpdatedInitial) {
			if (typeof this.props.initial === 'number') {
				...
				if (index >= 0 && index < this.props.data.length) this.setState({ index: index + 1, isUpdatedInitial: true });
			}
		}
	...
	onRelease(event) {
		...
		if (this.props.range) this.props.onIndexChange(this.state.index);
		else if (this.props.data) this.props.onLabelChange(this.props.data[this.state.index - 1]);
		event.stopPropagation();
		event.preventDefault();
	}
	onMove(event, pressPosition, isRelative) {
		...
		if (newPosition > rightEdge) newPosition = rightEdge;
		if (this.props.isSticky) newPosition = Math.round(newPosition / this.state.segmentWidth) * this.state.segmentWidth;
		this.setState({
	...
	onLabelChange(index) {
		if (this.props.range) this.props.onIndexChange(index + 1);
		else if (this.props.data) this.props.onLabelChange(this.props.data[index]);
		this.setState({ isLabelClick: true }, () => this.onMove(null, index * this.state.segmentWidth, true));
	}
	transformLabel(index) {
		if (this.props.data) {
			if (index > 0 && index < this.state.safeRange - 1)
				// Labels in the middle
				return 'translateX(-40%)';
			else if (index === this.state.safeRange - 1)
				// Label at the end
	...
	render() {
		if (!this.props.uniqueId) return <div>No unique ID provided!</div>;
		else if (
			!(this.props.data || this.props.range) ||
			((this.props.data && this.props.data.length < 2) || (this.props.range && this.props.range.length < 2))
		)
			...
			return (
				<div style={css.wrapper}>
					<div style={css.slider} id={this.props.uniqueId + '_track'} className="slider-lables-wrapper">
						<div style={css.track} onTouchStart={event => this.onPress(event)} onMouseDown={event => this.onPress(event)}>
							<div
								style={{
									...css.left,
					...
					{this.props.isSideLabel && this.state.index ? (
						<div style={css.sideLabel}>{this.props.data ? this.props.data[this.state.index - 1].label : this.state.index}</div>
					) : null}
				</div>
			);
		}
